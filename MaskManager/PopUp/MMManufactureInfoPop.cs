...
/// <summary>
///  * 파 일 명 : MMManufactureInfoPop.cs 					
///  * 작 성 자 : 황지희					
///  * 설     명 : Mask 생산정보입력
///  * 이     력 : 2019-05-03 [황지희] : 신규추가	
/// </summary>
namespace MaskManager.PopUp
{
    public partial class MMManufactureInfoPop : ParentsPop
    {
        ...
        {
            Load += MMManufactureInfoPop_Load;
            ...
            txt_NowUse.txtText.KeyPress += txt_KeyPress;  //총사용횟수 숫자입력 valid 체크 
            txt_EqpId.txtText.KeyDown += TxtText_KeyPress;
            txt_EqpId.txtText.KeyPress += txt_KeyPress;
        ...
        private void InitReset()
        {
            ...
            this.txt_MaskProdInfo.txtText.ResetText(); //Mask 모델정보
            QueryList = null;
            iNewTotUseQty = 0;
        }

        private void MMManufactureInfoPop_Load(object sender, EventArgs e)
        {
            //db connect
            DB.Open();
            
            //2019-05-14 hjh 설비 코드로 focus 되도록 변경
        ...
        public string  Valid()
        {
            ...
            int iLimitUseQty, iTotUseQty, iNowUseQty, iValidQty = 0;

            iLimitUseQty = int.Parse(ValidRow["LIMITUSEQTY"].ToString()); //한계 횟수
            iTotUseQty   = int.Parse(ValidRow["TOTUSEQTY"].ToString());   //누적 횟수
            iNowUseQty = int.Parse(txt_NowUse.txtText.Text.ToString());   //입력된 사용횟수
            iValidQty = iLimitUseQty - iTotUseQty;
            iNewTotUseQty = iTotUseQty + iNowUseQty;  //기존 누적 횟수 + 입력된 사용횟수 

            //2019-05-09 황지희 1.사용횟수 0이상 입력하도록 변경
            //                  2.기존 누적횟수 + 신규로 입력된 사용횟수가 한도횟수보다 많을경우 LOT TEMINATED 여부 결정 후 진행하도록 변경 
            
            if (iNowUseQty == 0 || iNowUseQty < 1) {
                CustomMessageBox.Show(MessageBoxButtons.OK, "ERROR", txt_NowUse.ucLabelText + "를 0이상 입력하세요.");
                txt_NowUse.txtText.Focus();
                txt_NowUse.txtText.SelectAll();
                return "SAVE_FALSE";
            }else { 
                if (iNowUseQty >= iValidQty) {
        ...
        private void TxtText_KeyPress(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                ...
                else
                {
                    ...
                    Params.Add(new SqlParameter("@equipmenttype", "EQUIPMENT_EQP"));
